...
notes: |
  2.1.0 (2026-04-14): 새 공식 출처 4종 추가 — Agent Teams, Agent Skills Engineering Blog, Claude Agent SDK, Code Execution with MCP. Progressive disclosure / gather-act-verify / code-based orchestration 원칙 반영.
...
- **Sub-agent로 연구/탐색/리뷰 위임.** 메인 세션 컨텍스트를 보호한다. Sub-agent는 "returns only the summary". [5]
...
- **Skill 위치 규약.** `.claude/skills/<name>/SKILL.md`. 파괴적 슬래시 커맨드는 `disable-model-invocation: true`로 사용자 수동 호출만 허용. SKILL.md는 500라인 이하 권장. [7]
...
- **2회 수정 실패 → 컨텍스트 리셋.** "If you've corrected Claude more than twice on the same issue in one session, the context is cluttered with failed approaches. Run `/clear` and start fresh." [8]
...
- **5가지 관리 패턴**: (a) CLAUDE.md 정적 지식, (b) 필요 시 로드(JIT), (c) `/clear`·`/compact`, (d) sub-agent 격리, (e) 파일시스템을 외부 메모리로. [8][13]
...
- **캐시 비용.** 읽기 = 기본 입력가 × 0.1, 5분 TTL 쓰기 = × 1.25, 1시간 TTL 쓰기 = × 2. 모델별 최소 캐시 토큰: Opus 4.6/4.5/Haiku 4.5 = 4096, Sonnet 4.6/Haiku 3.5/3 = 2048, Sonnet 4.5/Opus 4.1/4/Sonnet 4/3.7 = 1024. [4]
...
---
name: skill-name
description: 한국어 설명 (자동 로드 판단용. 250자 이내 front-load)
...
`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 설정 시 활성화. Lead 1 + Teammates N + 공유 task list + mailbox. Teammate 정의는 sub-agent definition 재사용 가능 (단 `skills`·`mcpServers` frontmatter는 적용되지 않음).
